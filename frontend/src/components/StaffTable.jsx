...
const StaffTable = ({ onEdit }) => {
  ...
  const filteredStaff = staffDevices.filter(staff =>
          ...
          <tbody>
            {filteredStaff.map(staff => (
              <tr key={staff._id}>
                ...
                <td>{staff.dateChecked ? new Date(staff.dateChecked).toLocaleDateString() : '-'}</td>
                <td>{staff.checkedBy || '-'}</td>
                <td style={{ maxWidth: '150px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{staff.remarks || '-'}</td>
                <td className="action-buttons">
                  <button onClick={() => onEdit(staff)} className="btn-edit">Edit</button>
                  <button onClick={() => setDeleteConfirm(staff._id)} className="btn-danger">Delete</button>
                </td>
              </tr>
            ))}
            {filteredStaff.length === 0 && (
              <tr>
                <td colSpan="10" style={{ textAlign: 'center' }}>No staff records found</td>
              </tr>
            )}
          </tbody>
        </table>
